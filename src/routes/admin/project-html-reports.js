...
export async function adminProjectHtmlReportRoutes(app) {
  app.get(
  ...
  app.delete(
    ...
    { onRequest: [app.verifyJwt, app.requireOwner] },
    async (request, reply) => {
      const deleted = await deleteProjectHtmlReport(request.params.id);
      if (!deleted) return reply.status(404).send({ message: 'Report not found' });
